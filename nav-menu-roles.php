	...
	class Nav_Menu_Roles {
		...
		*/
		public function register_importer() {
			...
			if ( defined( 'WP_LOAD_IMPORTERS' ) ) {
				...
				require_once plugin_dir_path( __FILE__ ) . 'inc/class-nav-menu-roles-import.php';
				// Register the custom importer we've created.
				$roles_import = new Nav_Menu_Roles_Import();

				register_importer( 'nav_menu_roles', __( 'Nav Menu Roles', 'nav-menu-roles' ), sprintf( __( 'Import %1$snav menu roles%2$s and other menu item meta skipped by the default importer', 'nav-menu-roles' ), '<strong>', '</strong>' ), array( $roles_import, 'dispatch' ) );
		...
		*/
		...
		<div class="field-nav_menu_role nav_menu_role_field description-wide" style="margin: 5px 0; <?php echo $hidden; ?>">
			...
			foreach ( $display_roles as $role => $name ) {
				...
				$checked = checked( true, ( is_array( $checked_roles ) && in_array( $role, $checked_roles, true ) ), false );
		...
		*/
		public function nav_update( $menu_id, $menu_item_db_id ) {
			...
			if ( isset( $_POST['nav-menu-logged-in-out'][ $menu_item_db_id ] ) && 'in' == $_POST['nav-menu-logged-in-out'][ $menu_item_db_id ] && ! empty( $_POST['nav-menu-role'][ $menu_item_db_id ] ) ) {
			...
			} elseif ( isset( $_POST['nav-menu-logged-in-out'][ $menu_item_db_id ] ) && in_array( $_POST['nav-menu-logged-in-out'][ $menu_item_db_id ], array( 'in', 'out' ), true ) ) {
